...
if [ -f ./md.cpt ]; then
    ...
    gmx mdrun -pin on -ntomp "$NP" -s md.tpr -cpi md.cpt -deffnm md -plumed plumed_restart.dat -maxh 36
else
    gmx grompp -f md.mdp -c ../npt/npt.gro -r ../npt/npt.gro -t ../npt/npt.cpt -p topol.top -o md.tpr
    gmx mdrun -pin on -ntomp "$NP" -deffnm md -plumed plumed.dat -maxh 36
fi