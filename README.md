...
Apart from the necessary python packages, this framework needs a `pymol` installation for PCC mutations and `acpype` to calculate GAFF2 parameters. The path to the `pymol` installation can be set through `FECalc/PCCBuilder_settings.JSON`, and GAFF2 parameters are generated by calling `acpype` directly from the code.
